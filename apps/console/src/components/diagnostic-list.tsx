                ...
                >
                    <div className="diagnostic-group-head">
                        ...
                        <span className="label">
                            {items.length}{" "}
                            {items.length === 1 ? "diagnostic" : "diagnostics"}
                        </span>
                    </div>
    ...
    return (
        <article className={`diagnostic ${diagnostic.severity ?? ""}`}>
            ...
            <div className="kv">
                <span>
                    rule{" "}
                    <span className="mono">{diagnosticRule(diagnostic)}</span>
                </span>
                <span>
                    stage{" "}
                    <span className="mono">
    ...
    switch (entity.kind) {
        case "variable":
            return text(entity.id)
                ? { kind: "variable", name: entity.id as string }
                : null;
        case "value":
        case "rule":
        ...
        case "qualifier":
            return text(entity.id)
                ? { kind: "qualifier", name: entity.id as string }
                : null;
        case "predicate":
        ...
        case "catalog":
            return text(entity.id)
                ? { kind: "catalog", name: entity.id as string }
                : null;
        case "catalog_entry":
        ...
        case "schema":
            return text(entity.path)
                ? {
                      kind: "schema",
                      name:
                          (entity.path as string).split("/").pop() ??
