...
export function DraftCandidates({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await apiFetch(`/api/workspaces/${workspaceId}/draft-candidates`);
        ...
        setScan({ kind: "done", candidates: body.candidates, skipped: body.skipped ?? 0 });
      } catch (error) {
        if (!cancelled) {
          setScan({
            kind: "error",
  ...
  async function openDraft(branch: string) {
    ...
    setOpenError(null);
    try {
      const response = await apiFetch(`/api/workspaces/${workspaceId}/drafts`, {
        method: "POST",
  ...
  return (
    <div className="card">
      <div className="card-head-text">
        ...
        <p className="hint">
          Discovered from GitHub: branches whose changes touch only this workspace. Open
          one to review and publish it from here.
        </p>
      </div>
      {scan.kind === "loading" ? (
        ...
        <>
            ...
            {scan.candidates.map((candidate) => (
              <div className="row" key={candidate.branch}>
                ...
                <span className="row-text">
                  <span className="row-title mono">{candidate.branch}</span>
                  <span className="row-sub">
                    {candidate.filesChanged}{" "}
                    {candidate.filesChanged === 1 ? "file" : "files"} changed · ahead by{" "}
                    {candidate.aheadBy}
                  </span>
                </span>
                <span className="row-side">
                  <button
                    ...
                    onClick={() => openDraft(candidate.branch)}
                    type="button"
                  >
                    {openingBranch === candidate.branch ? (
                      <span className="spin" />
                    ) : (
                      <GitBranchPlus aria-hidden size={14} />
                    )}
                    Open draft
                  </button>
                </span>
          ...
          {scan.skipped > 0 ? (
            <span className="field-hint">
              {scan.skipped} more {scan.skipped === 1 ? "branch was" : "branches were"} not
              scanned; open one by name below if it is missing here.
            </span>
          ) : null}
          {openError ? (
            <p className="form-note" data-tone="err">
