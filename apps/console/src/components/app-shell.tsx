    ...
    <div className="shell">
      <aside className="sidebar">
        ...
        <div className="side-user">
          {user.githubAvatarUrl ? (
            ...
            <img alt="" className="avatar" height={28} src={user.githubAvatarUrl} width={28} />
          ) : (
            <span className="avatar-fallback">{user.githubLogin.slice(0, 2)}</span>
          )}
      ...
      <div className="main" data-mode={editing ? "editing" : undefined}>
        <header className="topbar">
          <Link className="topbar-brand" href="/app" title="rototo admin">
            <RototoMark size={24} />
          </Link>
            ...
            <div className="crumbs">
              {crumbs.map((crumb, index) => (
                <Fragment key={`${crumb.label}-${index}`}>
                  ...
                  {crumb.href ? (
                    <Link className="label" href={crumb.href}>
                      {crumb.label}
                    </Link>
                  ) : (
        ...
        {editing ? (
          <div className="mode-strip">
            ...
            <span className="label mode-strip-label">draft editing</span>
            <span className="mono mode-strip-branch">{editing.label}</span>
            <span className="mode-strip-detail">{editing.detail}</span>
          </div>
        ) : null}
