...
export function ConsoleScreen({ screen }: { screen: AppScreen }) {
  ...
  const { data, error, loading, reload } = useApi<ConsoleData>("/api/console");
  const [workspaceSummaries, setWorkspaceSummaries] = useState<Map<string, WorkspaceSummary>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    for (const workspace of data.workspaces) {
      void api<WorkspaceSummary>(`/api/workspaces/${workspace.slug}/summary`).then(
        (summary) => {
          if (!cancelled) {
            setWorkspaceSummaries((current) => new Map(current).set(workspace.id, summary));
          }
        },
        (failure: unknown) => {
          if (!cancelled) {
