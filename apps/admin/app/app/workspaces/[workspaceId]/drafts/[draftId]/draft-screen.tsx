...
import type { WorkspaceSemanticModel } from "rototo";
import {
...
type DraftLintLoad =
  | Awaited<ReturnType<typeof lintInspectedWorkspace>>
...
export async function DraftScreen({
  ...
  let draft = getDraftSessionForUser(draftId, workspace.id, user.githubUserId);
  if (!draft) {
    notFound();
  }
  ...
  if (workspaceId !== workspace.slug) {
    redirect(
      path
        ? editEntityHref(workspace.slug, draftId, path)
        : screen === "edit"
          ? editKindHref(workspace.slug, draftId, requestedEditKind)
          : draftScreenHref(workspace.slug, draftId, selectedScreen),
    );
  }

  let prSyncError: string | null = null;
  const prNumber = draft.prNumber ?? pullRequestNumberFromUrl(draft.prUrl);
  const prSyncedRecently =
    draft.prSyncedAt !== null && Date.now() - Date.parse(draft.prSyncedAt) < 60_000;
  if (prNumber && !prSyncedRecently) {
    try {
      ...
      draft = updateDraftPullRequestState({
        draftId: draft.id,
        prNumber: pr.number,
      ...
      prSyncError = error instanceof Error ? error.message : String(error);
    }
  }

  const changes = listDraftChanges(draft.id);
  const events = listDraftEvents(draft.id);
  const activity = draftActivity(draft, events);

  let editableEntities: EditableEntity[] = [];
  let editLoadError: string | null = null;
  let draftModel: WorkspaceSemanticModel | null = null;
  let lint: DraftLintLoad;
  try {
    const inspected = await inspectWorkspace(draftLintTarget(workspace, draft), user.githubToken);
    draftModel = await semanticModelFor(inspected).catch(() => null);
    const [lintResult, entitiesResult] = await Promise.all([
      lintInspectedWorkspace(inspected).then(
        (loadedLint) => loadedLint,
        (error: unknown) => ({
          root: draft.branch,
          diagnostics: [],
      ...
      inspectWorkspaceInventory({
        ...
        async (inventory) => ({
          entities: await editableEntitiesFromInventory(workspace, inspected.root, inventory),
          error: null,
        }),
        (error: unknown) => ({
          entities: [],
    ...
    lint = lintResult;
    editableEntities = entitiesResult.entities;
    editLoadError = entitiesResult.error;
  } catch (error) {
  ...
  const lintHasErrors =
    "error" in lint || lint.diagnostics.some((diagnostic) => diagnostic.severity === "error");
  const selectedEntity = selectedEntityPath
    ? editableEntities.find((entity) => entity.path === selectedEntityPath) ?? null
    : null;
  const selectedEntityDiagnostics =
    selectedEntity && !("error" in lint)
      ? lint.diagnostics.filter((diagnostic) =>
          diagnosticMatchesEntity(diagnostic, selectedEntity, editableEntities),
        )
      : [];
  ...
  const contextPreviews: EditContextPreview[] =
    selectedEntity?.section === "variables"
      ? await draftContextPreviews({
          draft,
  ...
  const crumbs = [
    ...
    ...(selectedScreen !== "overview"
      ? [{ label: draft.branch, href: draftScreenHref(workspace.slug, draft.id, "overview") }]
      : []),
    ...
    <AppShell
      actions={
        <>
          <Link
            className="pill-link"
            href={draftScreenHref(workspace.slug, draft.id, "validate")}
      ...
      nav={
        <>
          <NavBack href={`/app/workspaces/${workspace.slug}/drafts`} label="Workspace" />
          <NavContext
            href={draftScreenHref(workspace.slug, draft.id, "overview")}
            label="draft branch"
            value={draft.branch}
          />
          ...
          <NavLink
            ...
            href={draftScreenHref(workspace.slug, draft.id, "changes")}
            icon={<GitCompare aria-hidden size={16} />}
            label="Changes"
          />
          <NavLink
            active={selectedScreen === "validate"}
            count={"error" in lint ? undefined : lint.diagnostics.length}
            href={draftScreenHref(workspace.slug, draft.id, "validate")}
          ...
          <NavLink
            ...
            href={draftScreenHref(workspace.slug, draft.id, "publish")}
            icon={<GitPullRequest aria-hidden size={16} />}
            label="Publish"
          />
          <NavGroupLabel>Edit</NavGroupLabel>
          <NavLink
            active={selectedScreen === "edit" && selectedEditKind === "variables"}
            count={editableEntityCounts.variables}
            href={editKindHref(workspace.slug, draft.id, "variables")}
            icon={<FileCode2 aria-hidden size={16} />}
            label="Variables"
          />
          <NavLink
            active={selectedScreen === "edit" && selectedEditKind === "qualifiers"}
            count={editableEntityCounts.qualifiers}
            href={editKindHref(workspace.slug, draft.id, "qualifiers")}
          ...
          <NavLink
            ...
            href={editKindHref(workspace.slug, draft.id, "schemas")}
            icon={<FileJson2 aria-hidden size={16} />}
            label="Schemas"
          />
          <NavLink
            active={selectedScreen === "edit" && selectedEditKind === "context"}
            count={editableEntityCounts.context}
            href={editKindHref(workspace.slug, draft.id, "context")}
            icon={<Braces aria-hidden size={16} />}
            label="Context"
          />
          <NavLink
            active={selectedScreen === "edit" && selectedEditKind === "linters"}
            count={editableEntityCounts.linters}
            href={editKindHref(workspace.slug, draft.id, "linters")}
      ...
      {selectedScreen === "changes" ? (
        <DraftChangesScreen
          ...
          entityHrefForFile={(filePath) => {
            const match = entityForDiagnosticPath(editableEntities, filePath);
            return match
              ? editEntityHref(workspace.slug, draft.id, match.path)
              : null;
          }}
        />
      ) : null}
      {selectedScreen === "validate" ? (
        <DraftValidateScreen
          diagnosticHref={(diagnostic) =>
            diagnosticEntityHref(diagnostic, editableEntities, workspace.slug, draft.id)
          }
          lint={lint}
        />
      ) : null}
    ...
    <section className="section">
      <div className="section-header">
        <div className="section-header-text">
          ...
          <p className="hint">
            Edits commit directly to this branch. When the draft is ready, publish it as
            a pull request from the publish screen.
          </p>
        </div>
        <DraftStatusPill draft={draft} />
      </div>
      <div className="meta-grid">
        ...
        <div className="meta-item">
          ...
          <span className="meta-value">{formatDate(draft.updatedAt)}</span>
        </div>
      </div>
      <div className="card">
        <div className="card-head-text">
          <h3>Branch name</h3>
          <p className="hint">
            Renaming moves the branch on GitHub. Locked once the draft is published.
          </p>
        </div>
        <DraftBranchEditor
          branch={draft.branch}
      ...
      <div className="section-header" style={{ marginTop: 8 }}>
        <div className="section-header-text">
          <h2>Activity</h2>
          <p className="hint">Everything that happened on this draft, newest first.</p>
        </div>
      </div>
      <div className="timeline">
        {newestFirst.map((event) => (
          <div className="tl-row" key={event.id}>
            <span className="tl-icon" data-tone={eventTone(event.kind)}>
              {eventIcon(event.kind)}
            </span>
            <span className="tl-body">
              <span>{event.summary}</span>
              {event.detailJson ? (
                <span className="tl-detail" title={event.detailJson}>
                  {event.detailJson}
                </span>
              ) : null}
            </span>
            <span className="tl-when">{formatDate(event.createdAt)}</span>
          </div>
        ))}
      </div>
    </section>
...
function DraftEditScreen({
  ...
  const entities = editableEntities.filter((entity) => entity.section === selectedKind);
      ...
      {selectedEntity ? (
        ...
        <>
          <div className="section-header-text">
            ...
            <p className="hint">
              Pick an entity to edit it with a form or as source. Saves commit to the
              draft branch.
            </p>
          </div>
          <EditableEntityList
...
function EditableEntityList({
  ...
  return (
    <>
      <AddEntityForm disabled={disabled} draftId={draftId} kind={kind} workspaceId={workspaceId} />
      {entities.length === 0 ? (
        <div className="empty-state">
          <span className="empty-puck">{editKindIcon(kind, 18)}</span>
          <p>No {EDIT_KIND_TITLES[kind].toLowerCase()} on this branch yet.</p>
        </div>
      ) : (
        <SearchableList
          className="row-list"
        ...
        >
          {entities.map((entity) => (
            <Link
              ...
              href={editEntityHref(workspaceId, draftId, entity.path)}
              key={entity.path}
            >
              <span className="row-icon">{editKindIcon(entity.section, 16)}</span>
              <span className="row-text">
                <span className="row-title mono">{entity.id}</span>
                <span className="row-sub">{entity.description ?? entity.path}</span>
              </span>
              <span className="row-side">
                {entity.badge ? <span className="tag">{entity.badge}</span> : null}
                <Pencil aria-hidden className="muted" size={14} />
              </span>
            </Link>
          ))}
        </SearchableList>
...
function EditableEntityDetail({
  ...
  const sourceMarks = diagnostics.flatMap((diagnostic) => {
    ...
    if (
      path === undefined ||
      line === undefined ||
      !(entity.path === path || entity.path.endsWith(`/${path}`) || path.endsWith(`/${entity.path}`))
    ) {
      return [];
    }
    return [
      {
        ...
        line: line + 1,
        severity: (diagnostic.severity === "error" ? "error" : "warning") as
          | "error"
          | "warning",
      },
    ];
      ...
      <div className="section-header">
        ...
        <div className="action-row">
          ...
          {parentResource ? (
            ...
            <Link
              className="btn btn-secondary btn-sm"
              href={editKindHref(workspaceId, draft.id, entity.section)}
            >
              <ArrowLeft aria-hidden size={14} />
      ...
      <div className="card">
        <div className="card-head">
          <div className="card-head-text">
            ...
            <p className="hint">
              Removes <span className="mono">{entity.path}</span> from the draft branch.
              The base ref is untouched until the pull request merges.
            </p>
          </div>
          <DeleteEntityButton
            disabled={draft.status !== "open"}
            ...
            returnHref={editKindHref(workspaceId, draft.id, entity.section)}
            workspaceId={workspaceId}
          />
        </div>
      </div>
...
function DraftChangesScreen({
  ...
  changes: ReturnType<typeof listDraftChanges>;
  ...
  return (
    <section className="section">
      <div className="section-header-text">
        <h1>Changes</h1>
        <p className="hint">
          Semantic changes tracked on this draft. They become the pull request body, so
          reviewers see what changed in rototo terms — not just file diffs.
        </p>
      </div>
      {changes.length === 0 ? (
        <div className="empty-state">
          ...
          <p>No tracked changes yet. Edits you save will show up here as a diff.</p>
        </div>
      ) : (
        <SearchableList
          className="row-list"
            ...
            >
              <div className="diffhead">
                <span className="diffhead-path">
                  {entityHrefForFile(change.filePath) ? (
                    <Link href={entityHrefForFile(change.filePath) as string}>
                      {change.variableId}
                    </Link>
                  ) : (
                    change.variableId
              ...
              <div className="diffbody">
                <div className="dl dl-del">
                  ...
                  <span className="t">{jsonSummary(change.beforeJson)}</span>
                </div>
                <div className="dl dl-add">
                  <span className="g">+</span>
                  <span className="t">{jsonSummary(change.afterJson)}</span>
                </div>
              </div>
            </article>
          ))}
  ...
  return "error" in lint ? (
    ...
    <section className="section">
      <div className="section-header-text">
        <h1>Validate</h1>
        <p className="hint">
          Lint runs against the draft branch. Publishing is blocked while errors are
          present — warnings ship, errors don’t.
        </p>
      </div>
      <DiagnosticList diagnosticHref={diagnosticHref} diagnostics={lint.diagnostics} />
    </section>
  );
}
    ...
    <section className="section">
      <div className="section-header-text">
        ...
        <p className="hint">
          ...
          <span className="mono">{draft.baseRef}</span>. Nothing reaches the base ref
          without review.
        </p>
      </div>
      {published ? (
        <div className="card">
          <div className="applied-row">
            ...
            <div className="card-head-text">
              ...
              <p className="hint">Review and merge on GitHub. State syncs back here.</p>
            </div>
            <div className="action-row" style={{ marginLeft: "auto" }}>
              <a className="btn btn-primary" href={draft.prUrl ?? "#"} rel="noreferrer" target="_blank">
                <ExternalLink aria-hidden size={15} />
                Open pull request
              </a>
              <SyncPrButton draftId={draft.id} workspaceId={workspaceId} />
            </div>
          </div>
          {prSyncError ? (
        ...
        <div className="card">
          ...
          <PublishCheck ok={!lintHasErrors}>
            {lintHasErrors ? "Lint reports errors — fix them on the validate screen" : "Lint is clean"}
          </PublishCheck>
          <PublishCheck ok={draft.status === "open"}>
            {draft.status === "open" ? "Draft is open" : `Draft is ${draft.status}`}
          </PublishCheck>
          <div className="action-row">
            <PublishDraftButton
              disabled={draft.status !== "open" || changesCount === 0 || lintHasErrors}
              draftId={draft.id}
              workspaceId={workspaceId}
            />
          </div>
...
function PublishCheck({ children, ok }: { children: ReactNode; ok: boolean }) {
  return (
    <div className="publish-check" data-ok={ok}>
      {ok ? <CheckCircle2 aria-hidden size={16} /> : <Circle aria-hidden size={16} />}
      <span>{children}</span>
    </div>
  );
}
...
async function editableEntitiesFromInventory(
  workspace: WorkspaceRecord,
  root: string,
  inventory: WorkspaceInventory,
): Promise<EditableEntity[]> {
  const nodes = [
    ...
    ...(inventory.context.schemaPath
      ...
      : []),
    ...inventory.context.examples.map((path) => ({
      section: "context" as const,
...
/* Evaluates every workspace qualifier against each saved request context via
   the Rust runtime. The runtime prefers the draft branch (so qualifier edits
   in the draft count) and falls back to the base workspace when the draft is
   not lint-clean. */
async function draftContextPreviews(input: {
  draft: DraftSessionRecord;
  entities: EditableEntity[];
  githubToken: string;
  workspace: WorkspaceRecord;
}): Promise<EditContextPreview[]> {
  const contexts = input.entities.filter((entity) => entity.kind === "context example");
  const qualifierIds = input.entities
    .filter((entity) => entity.kind === "qualifier")
    .map((entity) => entity.id);
  if (contexts.length === 0 || qualifierIds.length === 0) {
    return [];
  }
  let runtime: Awaited<ReturnType<typeof loadWorkspaceRuntime>>;
  try {
    runtime = await loadWorkspaceRuntime(draftLintTarget(input.workspace, input.draft), input.githubToken);
  } catch {
    try {
      runtime = await loadWorkspaceRuntime(input.workspace, input.githubToken);
    } catch {
      return [];
    }
  }
  const previews: EditContextPreview[] = [];
  for (const context of contexts.slice(0, 4)) {
    let parsed: Record<string, unknown>;
    try {
      parsed = JSON.parse(context.text) as Record<string, unknown>;
    } catch {
      continue;
    }
    const qualifierTruth: Record<string, boolean> = {};
    for (const qualifierId of qualifierIds) {
      try {
        qualifierTruth[qualifierId] = (
          await runtime.resolveQualifier(qualifierId, parsed as never)
        ).value;
      } catch {
        // not evaluable against this context; the preview reports it as such
      }
    }
...
function draftActivity(
  ...
  const hasCreatedEvent = events.some((event) => event.kind === "draft.created");
  if (hasCreatedEvent) {
    return events;
  }
  return [
    {
      ...
      detailJson: JSON.stringify({ branch: draft.branch, baseRef: draft.baseRef }),
...
function editKindHref(workspaceId: string, draftId: string, kind: EditKind): string {
  return `/app/workspaces/${workspaceId}/drafts/${draftId}/edit/${kind}`;
}

function editEntityHref(workspaceId: string, draftId: string, path: string): string {
  return `/app/workspaces/${workspaceId}/drafts/${draftId}/tree/${encodeEntityPath(path)}`;
}

export function normalizeDraftScreen(value: string | null): DraftScreenId | null {
  if (
    value === "overview" ||
    value === "edit" ||
    value === "changes" ||
...
function editableEntitySearchText(entity: EditableEntity): string {
  return [entity.id, entity.kind, entity.path, entity.description, entity.badge]
    .filter(Boolean)
    .join(" ");
}
...
function contextAttributeSuggestions(entities: EditableEntity[]): string[] {
  const contextSchema = entities.find((entity) => entity.path.endsWith("schemas/context.schema.json"));
  if (!contextSchema) {
    return [];
  }
  try {
...
function buildFormGuidance(
  ...
  if (entity.section === "qualifiers") {
    ...
    for (const qualifier of model?.qualifiers ?? []) {
      ...
      for (const predicate of qualifier.predicates) {
        ...
        if (!subject || subject.startsWith("qualifier.") || predicate.value === undefined) {
          continue;
        }
  ...
  if (entity.kind === "resource object") {
    ...
    for (const sibling of model?.resourceObjects ?? []) {
      ...
      const fields =
        typeof sibling.value === "object" && sibling.value !== null && !Array.isArray(sibling.value)
...
function contextAttributeDocs(entities: EditableEntity[]): Record<string, string> {
...
function resourceObjectSchemaText(
  ...
  const schema = entities.find(
    (candidate) => candidate.section === "schemas" && candidate.path.split("/").pop() === basename,
  );
...
function diagnosticMatchesEntity(
  ...
  if (entityForDiagnosticPath(entities, diagnostic.location?.path)?.path === entity.path) {
    return true;
  }
...
function diagnosticEntityHref(
  ...
  const pathMatch = entityForDiagnosticPath(entities, diagnostic.location?.path);
  if (pathMatch) {
...
function entityFromSemanticTarget(
  ...
  if (entity.kind === "variable" && typeof entity.id === "string") {
    return entities.find((candidate) => candidate.section === "variables" && candidate.id === entity.id) ?? null;
  }
  if (
    (entity.kind === "value" || entity.kind === "rule") &&
    typeof entity.variable === "string"
  ) {
    return entities.find((candidate) => candidate.section === "variables" && candidate.id === entity.variable) ?? null;
  }
  if (entity.kind === "qualifier" && typeof entity.id === "string") {
    return entities.find((candidate) => candidate.section === "qualifiers" && candidate.id === entity.id) ?? null;
  }
