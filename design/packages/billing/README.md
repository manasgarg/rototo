...
| Use case | Where |
| --- | --- |
| 4 Plan to entitlements | `variables/plan_entitlements.toml` (query + map projection) |
| 4 Plan limits and quotas | `variables/seat_limit.toml`, `variables/api_rate_limit_per_min.toml` |
...
- `(team, eur)` resolves `active_price` to `team_eur_2025`; `(team, usd)`
  resolves to `team_usd_2025` before 2026-10-01 and `team_usd_2026_10` after,
  from the same package version.
- `plan_entitlements` for business includes `sso`; for free it is exactly
  `["api_access"]`. `"sso" in variables.plan_entitlements` is expressible.
- A price entry referencing an unknown tier or currency is a lint failure; a
  plan referencing an unknown feature is a lint failure.
- An overlay that adds a price entry or patches a plan is rejected by
  governance; an overlay that overrides `api_rate_limit_per_min` composes.
