...
- The same `user.id` gets the same arm on every resolution (deterministic
  assignment), and no `user.id` is in both the rollout and the experiment.
...
- `enable_v3_branding` is false before 2026-09-15T14:00:00Z and true after,
  from the same package version.
- Bucket ranges in `layers/editor.toml` that overlap are a lint failure; an
  assign referencing an unknown arm is a lint failure; an arm value outside the
  variable's enum is a lint failure.
