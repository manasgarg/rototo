...
function loadNative(): NativeModule {
  ...
  throw new Error(`failed to load rototo native module:\n${errors.join("\n")}`);
}
...
function nativeCandidates(): string[] {
  ...
  if (platform === "linux") {
    const linuxArch = arch === "x64" ? "x64" : arch === "arm64" ? "arm64" : arch;
  ...
  if (platform === "darwin") {
    const darwinArch = arch === "x64" ? "x64" : arch === "arm64" ? "arm64" : arch;
