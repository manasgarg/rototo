...
Rototo's `when` and `query` expressions are a CEL subset: the `cel` crate
(pinned `=0.14.0`) does the parsing and evaluation, and rototo owns the
...
|---|---|---|---|
| R1 | `context`, `entry`, `variables["<id>"]`, `env.now` | all four roots evaluate; no issues reported | `flags_invalid_expression_roots` (the valid arm), `evaluates_context_paths_entry_paths_and_variables`, `evaluates_env_members` |
