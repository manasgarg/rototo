...
| R3 | registers a target outside the address grammar (legacy `/...` spellings, `#` pointers, untargetable classes, malformed addresses) | rejected with the target named and a hint; the grammar itself lives in `tests/docs/addressing-matrix.md` | `reports_custom_registration_contract_failures` (`tests/package_lint.rs`), the acceptance tests in `src/lint/custom/registry.rs` |
