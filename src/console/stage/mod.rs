...
mod tests {
    ...
    async fn base_workspace_selector_adapts_current_github_store_shape() {
        ...
        assert_eq!(
            ...
            WorkspaceSelector {
                ...
                .unwrap(),
                path: WorkspacePath("workspaces/payments".to_owned()),
                selection: SourceTreeSelection::BaseRef(GitRefName("main".to_owned())),
            }
        );
    }
    ...
    async fn branch_workspace_selector_adapts_token_and_branch_identity() {
        ...
        assert_eq!(
            selector.selection,
            SourceTreeSelection::Branch(BranchName(
                "rototo-console/alice/change-checkout".to_owned()
            ))
        );
    }
    ...
    async fn selector_adapter_preserves_generic_git_remote_identity() {
        ...
        assert_eq!(selector.path, WorkspacePath("services/api".to_owned()));
        assert_eq!(
            selector.selection,
            SourceTreeSelection::BaseRef(GitRefName("main".to_owned()))
        );
    }
