...
fn typed_operand_completion_items(
    snapshot: &PackageLintSnapshot,
    path: &str,
...
fn comparison_lhs_path<'a>(prefix: &'a str, token: &str) -> Option<(&'a str, Vec<&'a str>)> {
...
fn schema_ref_pin(schema: &JsonValue, segments: &[&str]) -> Option<SchemaRefPin> {
    let mut current = schema;
...
fn enum_member_literal_items(index: &SemanticIndex, id: &str) -> Vec<PackageCompletionItem> {
