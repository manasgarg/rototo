...
Applications and CLI commands do not have to read a package from the local
filesystem. They load a package source. That source is the deployment handle:
...
[package layering](reference-package-layering.html).
...
| `git+file://` | `git+file:///repos/config.git#main:package` | Clones a local git repository. |
...
| HTTPS archive | `https://example.com/config.tar.gz#:package` | Downloads and extracts an archive over HTTPS. |
...
git+https://github.com/acme/config.git#main:packages/prod
git+https://github.com/acme/config.git#:packages/prod
```

The part before `:` is the git ref. The part after `:` is the package
subdirectory inside the checkout. If no ref is supplied, git's default checkout
is used. If no subdirectory is supplied, the checkout root is the package.
...
https://example.com/config.tar.gz#:packages/prod
```

Archive sources do not support a ref. If an archive contains more than one
possible package root, pass `#:subdir` so rototo can stage the intended
package.

Archive downloads follow HTTPS-only redirects. Bearer auth is available through
the CLI `--package-token` flag, the `ROTOTO_PACKAGE_TOKEN` environment
...
points directly at the package root.
...
| `PackageLayers` | Layered package composed from multiple sources. |

Refresh compares fingerprints before replacing the active package. See
