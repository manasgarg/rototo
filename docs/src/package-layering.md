...
Putting all of that in one package forces the wrong tradeoff. Either every
owner can edit too much, or the product has to fork configuration for every
customer. [Package layering](reference-package-layering.html) gives us a
better boundary: each owner edits the package they are responsible for, while
the final package inherits the contracts and values below it.
...
- the app loads the team package and resolves the final policy.
...
the layers into one package, lints the result, and resolves variables from
that final package.
...
customer-config/
  rototo-package.toml  # extends product-config
  catalogs/
  variables/

team-config/
  rototo-package.toml  # extends customer-config
  qualifiers/
  catalogs/
  variables/
  request-contexts/
...
[package source](reference-package-sources.html) it is allowed to use. In
this example, that is `team-config`. Rototo follows the `extends` chain and
builds the inherited package before lint and resolution.
...
Create `product-config/rototo-package.toml`:
...
Now create a customer package that
[extends](reference-package-manifest.html) the product layer:

```toml
# customer-config/rototo-package.toml
schema_version = 1
...
thumb for [layered packages](reference-package-layering.html): when a later
...
Now create a team package that extends the customer layer:

```toml
# team-config/rototo-package.toml
schema_version = 1
...
Because the qualifier introduced `task.kind`, the team package needs the
...
variable from the final package.

## What The App Loads

The app should [load the most specific package source](reference-sdk-loading.html):

:::sdk-snippet package-layering-app
```rust
...
async fn route_for_task(
    ...
    let pkg = Package::load(package_source).await?;
...
```java
Object routeForTask(String packageSource, String taskKind) throws Exception {
    try (Package pkg = Package.load(packageSource).get()) {
...
func routeForTask(
    ...
    pkg, err := rototo.Load(ctx, packageSource, nil)
    if err != nil {
        return nil, err
    }
    defer pkg.Close()
...
For this example, `package_source` would point at `team-config`. In a hosted
setup, it could be a git source for the team package. That source can extend
the customer package, which can extend the product package. Rototo loads
the graph, projects the inherited files, lints the projected package, and
...
team package can pick up the new projected package after a successful
refresh. If refresh fails, the last successfully loaded package stays active.

## Keep The Boundaries Honest

Use package layering when separate owners need to share one configuration
model:
...
- temporary incident policy layered over a normal operating package.

Do not use layering to hide authorization problems. Rototo will merge and lint
the package graph, but it does not decide who is allowed to edit each layer.
...
to change? If the answer is clear, layering gives that owner a package
